...
extern "C" {
	...
	VOID KdCreateDeviceSymbolLink(PDRIVER_OBJECT driverObj) {
		...
		if (!NT_SUCCESS(status)) {
			DbgPrint("There was a problem creating the device IOCTL");
			return;
		}
		...
		if (!NT_SUCCESS(status)) {
			DbgPrint("There was a problem setting a link to the device IOCTL");
			return;
		}
		...
		DbgPrint("Success Creating DEVICE IOCTL");
	}
	...
	NTSTATUS KdScanProcesso(const char* NomeProcesso) {
		...
		{
			DbgPrintEx(0, 0, "MHYPROT2 -> Path: %s \n", module[i].FullPathName);
			DbgPrintEx(0, 0, "COMPARING WITH -> %s \n", NomeProcesso);
			if (!strcmp((char*)module[i].FullPathName, NomeProcesso)) 
			{
				DbgPrintEx(0, 0, "module found !");
				BaseProcesso = (UINT64)module[i].ImageBase; 
				TamanhoProcesso = (UINT64)module[i].ImageSize; 
				break;
			}
		}
